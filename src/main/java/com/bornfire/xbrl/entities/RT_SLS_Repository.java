...
public interface RT_SLS_Repository extends JpaRepository<RT_SLS_ENTITIES, String> {
	...
	@Query(value = "SELECT * FROM RT_SLS_USD WHERE TRUNC(REPORT_DATE) = (SELECT TRUNC(MAX(REPORT_DATE)) FROM RT_SLS_USD) "
			+ "AND TO_NUMBER(SRL_NO) = (SELECT MAX(TO_NUMBER(SRL_NO)) FROM RT_SLS_USD "
			+ "WHERE TRUNC(REPORT_DATE) = (SELECT TRUNC(MAX(REPORT_DATE)) FROM RT_SLS_USD)) AND ROWNUM = 1",
			nativeQuery = true)
	Optional<RT_SLS_ENTITIES> findLatestReport();

	@Query(value = "SELECT DISTINCT TRUNC(REPORT_DATE) FROM RT_SLS_USD ORDER BY 1 DESC", nativeQuery = true)
	List<Date> findDistinctReportDates();

	@Query(value = "SELECT * FROM RT_SLS_USD WHERE TRUNC(REPORT_DATE) = TRUNC(?1) AND REPORT_CURRENCY = ?2 "
			+ "AND TO_NUMBER(SRL_NO) = (SELECT MAX(TO_NUMBER(SRL_NO)) FROM RT_SLS_USD "
			+ "WHERE TRUNC(REPORT_DATE) = TRUNC(?1) AND REPORT_CURRENCY = ?2) AND ROWNUM = 1",
			nativeQuery = true)
	Optional<RT_SLS_ENTITIES> findLatestByDateAndCurrency(Date reportDate, String currency);
	 
	@Query(value = "select * from RT_SLS_USD order by REPORT_DATE", nativeQuery = true)
	 List<RT_SLS_ENTITIES> rtslslist();
	 
	 @Query(value = "select * from RT_SLS_USD where TRUNC(REPORT_DATE)=TRUNC(?1) AND REPORT_CURRENCY=?2 ", nativeQuery = true)
	 List<RT_SLS_ENTITIES> rtslslistbydate(Date reportdate,String currency);
	 
	 @Query(value = "select * from RT_SLS_USD where TRUNC(REPORT_DATE)=TRUNC(?1)", nativeQuery = true)
