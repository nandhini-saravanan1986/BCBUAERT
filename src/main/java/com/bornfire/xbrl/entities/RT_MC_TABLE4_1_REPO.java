...
public interface RT_MC_TABLE4_1_REPO extends JpaRepository<RT_MC_TABLE4_1_ENTITY, MCReportId> {
	...
	@Query(value = "SELECT * FROM RT_MC_TABLE4_1 WHERE REPORT_DATE = TO_DATE(:reportDate, 'DD-MM-YYYY') AND BRANCH_CODE <> 'DEPT' ORDER BY REPORT_DATE DESC FETCH FIRST 1 ROWS ONLY", nativeQuery = true)
	List<RT_MC_TABLE4_1_ENTITY> findByReportDate(@Param("reportDate") String reportDate);

	@Query(value = "SELECT * FROM RT_MC_TABLE4_1 WHERE BRANCH_CODE = :branch ORDER BY REPORT_DATE DESC FETCH FIRST 1 ROWS ONLY", nativeQuery = true)
	List<RT_MC_TABLE4_1_ENTITY> findBybranchcode(@Param("branch") String branch);

	@Query(value = "SELECT * FROM RT_MC_TABLE4_1 WHERE REPORT_DATE = TO_DATE(:reportDate, 'DD-MM-YYYY') AND BRANCH_CODE = :branchCode ORDER BY REPORT_DATE DESC FETCH FIRST 1 ROWS ONLY", nativeQuery = true)
	List<RT_MC_TABLE4_1_ENTITY> findByReportDateAndBranchCode(@Param("reportDate") String reportDate,
	...
	@Query(value = "SELECT * FROM RT_MC_TABLE4_1 WHERE REPORT_DATE IN ("
			+ " SELECT DISTINCT REPORT_DATE FROM RT_MC_TABLE4_1 "
	...
	List<RT_MC_TABLE4_1_ENTITY> findLastFourReports(@Param("reportDate") String reportDate,
	...
	@Query(value = "SELECT * FROM RT_MC_TABLE4_1 WHERE REPORT_DATE IN ("
			...
			+ " ORDER BY REPORT_DATE DESC", nativeQuery = true)
	List<RT_MC_TABLE4_1_ENTITY> findLastTwoReports(@Param("reportDate") String reportDate,
			@Param("timeperiod") String timeperiod);
	@Modifying
	@Transactional
	@Query("UPDATE RT_MC_TABLE4_1_ENTITY r SET r.ENTITY_FLG = :entityFlg, r.SIGNOFF_REMARKS = :signoffremarks "
	...
	@Query(value = "SELECT COUNT(*) FROM RT_MC_TABLE4_1 WHERE REPORT_DATE = TO_DATE(:reportDate, 'DD-MM-YYYY') AND BRANCH_CODE = :branchCode ", nativeQuery = true)
