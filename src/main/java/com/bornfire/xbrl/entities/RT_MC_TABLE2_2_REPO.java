...
public interface RT_MC_TABLE2_2_REPO extends JpaRepository<RT_MC_TABLE2_2_ENTITY, MCReportId> {
	...
	@Query(value = "SELECT * FROM RT_MC_TABLE2_2 WHERE BRANCH_CODE = :branch ORDER BY REPORT_DATE DESC FETCH FIRST 1 ROWS ONLY", nativeQuery = true)
	List<RT_MC_TABLE2_2_ENTITY> findBybranchcode(@Param("branch") String branch);

	@Query(value = "SELECT * FROM RT_MC_TABLE2_2 WHERE REPORT_DATE = TO_DATE(:reportDate, 'DD-MM-YYYY') AND BRANCH_CODE = :branchCode ORDER BY REPORT_DATE DESC FETCH FIRST 1 ROWS ONLY", nativeQuery = true)
	List<RT_MC_TABLE2_2_ENTITY> findByReportDateAndBranchCode(@Param("reportDate") String reportDate,
	...
	@Query(value = "SELECT * FROM RT_MC_TABLE2_2 WHERE REPORT_DATE IN ("
			+ " SELECT DISTINCT REPORT_DATE FROM RT_MC_TABLE2_2 "
	...
	List<RT_MC_TABLE2_2_ENTITY> findLastFourReports(@Param("reportDate") String reportDate,
