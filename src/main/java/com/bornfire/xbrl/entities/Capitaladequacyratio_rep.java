...
public interface Capitaladequacyratio_rep extends JpaRepository<Capitaladequacyratio_entity, Date> {
	...
	@Query(value="Select To_char(month_end) as month_end,R1_RATIOS2 from (\r\n"
			+ "						With Eligibility_ratio as(\r\n"
			+ "						Select (R1_RATIOS2) as R1_RATIOS2, REPORT_DATE from BRF95_SUMMARYTABLE),\r\n"
	...
	@Query(value = "Select To_char(month_dates) as month_end,R1_RATIOS2 from (\r\n"
			+ "						With Eligibility_ratio as(\r\n"
			+ "						Select (R1_RATIOS2) as R1_RATIOS2, REPORT_DATE from BRF95_SUMMARYTABLE),\r\n"
