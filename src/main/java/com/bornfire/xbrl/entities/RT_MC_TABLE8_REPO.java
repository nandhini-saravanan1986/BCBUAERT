...
public interface RT_MC_TABLE8_REPO extends JpaRepository<RT_MC_TABLE8_ENTITY, MCReportId> {
	...
	@Query(value = "SELECT * FROM RT_MC_TABLE8 WHERE BRANCH_CODE = :branch ORDER BY REPORT_DATE DESC FETCH FIRST 1 ROWS ONLY", nativeQuery = true)
	List<RT_MC_TABLE8_ENTITY> findBybranchcode(@Param("branch") String branch);

	@Query(value = "SELECT * FROM RT_MC_TABLE8 WHERE REPORT_DATE = TO_DATE(:reportDate, 'DD-MM-YYYY') AND BRANCH_CODE = :branchCode ORDER BY REPORT_DATE DESC FETCH FIRST 1 ROWS ONLY", nativeQuery = true)
	List<RT_MC_TABLE8_ENTITY> findByReportDateAndBranchCode(@Param("reportDate") String reportDate,
			@Param("branchCode") String branchCode);

	@Modifying
	@Transactional
	@Query("UPDATE RT_MC_TABLE8_ENTITY r SET r.VERIFY_FLG = :verifyFlg, r.REMARKS = :remarks "
	...
	@Query(value = "SELECT * FROM RT_MC_TABLE8 WHERE REPORT_DATE IN ("
			+ " SELECT DISTINCT REPORT_DATE FROM RT_MC_TABLE8 "
	...
	List<RT_MC_TABLE8_ENTITY> findLastFourReports(@Param("reportDate") String reportDate,
	...
	@Query(value = "SELECT * FROM RT_MC_TABLE8 WHERE REPORT_DATE IN ("
			...
			+ " ORDER BY REPORT_DATE DESC", nativeQuery = true)
	List<RT_MC_TABLE8_ENTITY> findLastTwoReports(@Param("reportDate") String reportDate,
			@Param("timeperiod") String timeperiod);

	@Modifying
	@Transactional
	@Query("UPDATE RT_MC_TABLE8_ENTITY r SET r.ENTITY_FLG = :entityFlg, r.SIGNOFF_REMARKS = :signoffremarks "
	...
	@Query(value = "SELECT COUNT(*) FROM RT_MC_TABLE8 WHERE REPORT_DATE = TO_DATE(:reportDate, 'DD-MM-YYYY') AND BRANCH_CODE = :branchCode ", nativeQuery = true)
